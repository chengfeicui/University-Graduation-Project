...
// 65、对设置的构造函数做一些基础的初始化即可
DataPackage::DataPackage()
{
    // 83、自定义的信号DataPackage data需要注册才能够使用
...
void DataPackage::setPackagePointer(const u_char *pkt_content,int size){
    // 67、指针这个地方要先申请一块空间，把对应的数据保留下来，因为后面还要动态的解析数据
...
QString DataPackage::getTcpSourcePort(){
    ...
    if(port == 8080) return "http(8080)";
    if(port == 3128) return "http(3128)";
    if(port == 8081) return "http(8081)";
    if(port == 9098) return "http(9098)";
...
QString DataPackage::getTcpOperationKind(int kind){
    ...
    case 2:return "MSS";
    case 3:return "WSOPT";
    case 4:return "SACK-Premitted";
    case 5:return "SACK";
    case 8:return "TSPOT";
    case 19:return "TCP-MD5";
    case 28:return "UTP";
    case 29:return "TCP-AO";
    }
}
